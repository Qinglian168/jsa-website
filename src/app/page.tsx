...
export default function HomePage() {
  return (
    <>
      {/* Hero Section - Dark Tech with Banner Image */}
      <section className="relative min-h-[600px] lg:min-h-[700px] flex items-center overflow-hidden scanlines">
        ...
        <div className="absolute inset-0" style={{background: "linear-gradient(135deg, rgba(7,11,30,0.92), rgba(10,14,39,0.85), rgba(124,58,237,0.15))"}} />
        {/* Grid pattern */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-32 relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 bg-cyan/10 backdrop-blur-sm border border-cyan/20 rounded-full px-4 py-1.5 text-sm mb-6">
                <span className="w-2 h-2 bg-cyan rounded-full pulse-dot" />
                <span className="text-cyan">20 Years of Trusted Network Equipment Distribution</span>
              </div>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold leading-tight mb-6 text-white">
                Your Trusted <br />
                <span className="text-gradient">Network Equipment</span>
                <br />Distributor
              </h1>
              ...
              <div className="flex flex-wrap gap-4">
                <Link
                  href="/products"
                  className="inline-flex items-center px-7 py-3.5 btn-neon rounded-lg text-sm"
                >
                  Browse Products
                ...
                <Link
                  href="/contact"
                  className="inline-flex items-center px-7 py-3.5 bg-white/5 backdrop-blur-sm border border-white/20 text-white font-semibold rounded-lg hover:bg-white/10 hover:border-cyan/30 transition-all text-sm"
                >
                  Request a Quote
                </Link>
              </div>
            ...
            <div className="hidden lg:block">
              <div className="relative">
                <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8" style={{boxShadow: "0 0 40px rgba(0,180,216,0.1)"}}>
                  <div className="grid grid-cols-2 gap-4">
                    {stats.map((stat) => (
                      <div key={stat.label} className="bg-white/5 border border-white/5 rounded-xl p-5 text-center">
                  ...
                  <div className="mt-6 bg-white/5 border border-white/5 rounded-xl p-4">
                    <div className="text-xs text-cyan/60 mb-2 uppercase tracking-wider">Trusted Brands</div>
                    <div className="flex flex-wrap gap-2">
                      {allBrands.slice(0, 12).map((brand) => (
                        <span key={brand} className="text-xs bg-white/5 border border-white/5 px-2 py-1 rounded text-white/60">{brand}</span>
                      ))}
                      <span className="text-xs bg-cyan/10 border border-cyan/20 px-2 py-1 rounded text-cyan">+23 more</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* Stats Mobile */}
      <section className="lg:hidden py-8 bg-light">
      ...
      {/* Product Categories with Banner Images */}
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories.map((cat) => (
              ...
              >
                ...
                <span className="inline-flex items-center text-sm font-semibold text-cyan">
                  Explore
      ...
      <section className="py-16 lg:py-20 bg-primary-dark">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {allBrands.map((brand) => (
              <Link
                key={brand}
                href="/brands"
                className="px-5 py-3 bg-surface border border-border rounded-xl text-sm font-semibold text-white/70 hover:border-cyan/40 hover:text-cyan hover:shadow-lg hover:shadow-cyan/5 transition-all duration-200"
              >
                {brand}
              </Link>
            ))}
          ...
          <div className="text-center mt-8">
            <Link href="/brands" className="text-cyan font-semibold text-sm hover:underline">
              View all 35+ brands →
            </Link>
          </div>
        </div>
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {solutions.map((sol) => (
              ...
              >
                ...
                <div>
                  <h3 className="text-lg font-bold text-white group-hover:text-cyan transition-colors mb-1">
                    {sol.title}
                  </h3>
                  <p className="text-sm text-gray leading-relaxed">{sol.desc}</p>
                  <span className="inline-flex items-center text-sm font-semibold text-cyan mt-3">
                    Learn More
                    <svg className="w-4 h-4 ml-1 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                  </span>
                </div>
      ...
      {/* Why Choose Us - Dark with glow */}
      <section className="py-16 lg:py-24 relative overflow-hidden" style={{background: "linear-gradient(135deg, #070b1e, #0d1b3e, #1a1040)"}}>
        {/* Glow accents */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {whyUs.map((item, i) => (
              <div key={i} className="bg-white/5 backdrop-blur-sm border border-white/5 rounded-2xl p-6 hover:bg-white/8 hover:border-cyan/20 transition-all">
      ...
      <section className="py-16 lg:py-20 bg-primary-dark">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link
              href="/contact"
              className="inline-flex items-center px-8 py-4 btn-neon rounded-lg text-sm"
            >
              Request a Quote
              <svg className="w-4 h-4 ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
            </Link>
            <Link
              href="/products"
              className="inline-flex items-center px-8 py-4 border border-white/20 text-white font-bold rounded-lg hover:bg-white/5 hover:border-cyan/30 transition-all text-sm"
            >
              Browse Products
            </Link>
          </div>
